...
def get_all(args):
    from scraper import get_all_interactives_list
    for story_id in get_all_interactives_list():
        archive(story_id)

def get_search(args):
    for url in args.urls:
        url = url.replace('"','')
        archive_all(search_string=url)
...
parser_all = subparsers.add_parser('all', help='download all stories')
parser_all.set_defaults(func=get_all)
...
if hasattr(args, "func"):
    ...
    print("Please specify an action of: {all, update, get, get_search}")
