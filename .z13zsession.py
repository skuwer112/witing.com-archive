...
def __log_in():
    ...
    login_p = browser.get("https://www.writing.com/")
    login_form = login_p.soup.find("form", action="/main/login.php", method="post")

    if login_form is None:
        raise Exception("Nie znaleziono formularza logowania na Writing.com")

    username_form = login_form.find("input", {"name": "login_username"})
    password_form = login_form.find("input", {"name": "login_password"})

    if username_form is None or password_form is None:
        raise Exception("Nie znaleziono pól loginu/hasła")
    ...
    if landing_p.soup.find("form", action="/main/login.php", method="post") is not None:
        print("Error logging in. Invalid credentials?")
        raise Exception("Logowanie nieudane")

    with open("session", "w") as f:
        json.dump(dict(browser.session.cookies), f)
